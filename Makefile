...
ifeq ($(CUDA),1)
    ...
    ifeq ($(TESLA),1)
        GENCODE_FLAGS = -gencode arch=compute_13,code=sm_13
    else
        GENCODE_FLAGS = -gencode arch=compute_20,code=sm_20 -gencode arch=compute_30,code=sm_30 -gencode arch=compute_35,code=sm_35 -gencode arch=compute_37,code=sm_37 -gencode arch=compute_50,code=sm_50 -gencode arch=compute_52,code=sm_52
    endif
