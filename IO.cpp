...
int printPDF(const double *PDF, unsigned int Nhist, double hist_bin, string name, unsigned int source, unsigned int PDFtype) {
	...
	string PDFtype_str;
	if (PDFtype == 0) PDFtype_str = "RDF";
	else if (PDFtype == 1) PDFtype_str = "PDF";
	else if (PDFtype == 2) PDFtype_str = "rPDF";
	if (source) outname << name << "_xray_" << PDFtype_str << ".txt";
...
int printPartialPDF(const double *PDF, unsigned int Nhist, double hist_bin, string name, unsigned int source, unsigned int PDFtype, unsigned int Ntype, map <string, unsigned int> ID) {
	...
	for (unsigned int iType = 0; iType < Ntype; iType++) {
		...
		for (unsigned int jType = iType; jType < Ntype; jType++, count += Nhist) {
			ostringstream outname;
			if (source) outname << name << "_xray_partial_" << PDFtype_str << "_" << it->first << "_" << jt->first << ".txt";
...
int printI(const double *I,unsigned int Nq,const double *q, string name, unsigned int source){
	...
	ostringstream outname;
	if (source) outname << name << "_xray_1D.txt";
...
int printPartialI(const double *I, unsigned int Nq, const double *q, string name, unsigned int source, unsigned int Nblocks){
	...
	for (unsigned int iB = 0; iB < Nblocks; iB++) {
		for (unsigned int jB = iB; jB < Nblocks; jB++, count += Nq) {
			ostringstream outname;
			if (source) outname << name << "_xray_" << iB << "-" << jB << "_1D.txt";
...
int printI2(const double * const *I, unsigned int Nq, unsigned int Nfi, string name, unsigned int source){
	...
	//raws - q; columns - fi
	ostringstream outname;
	if (source) outname << name << "_xray_2D.txt";
...
int printAtoms (const vector < vect3d<double> > *ra,string name,map <string, unsigned int> ID, unsigned int Ntot) {
	...
	if (out.is_open()){
		...
		string TypeName;
		map <string, unsigned int>::iterator it = ID.begin();
		for (unsigned int iType=0;iType<ID.size();iType++){			
			TypeName = it->first;
