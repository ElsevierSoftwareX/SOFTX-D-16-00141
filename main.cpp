...
int printPartialPDF(const double *PDF, unsigned int Nhist, double hist_bin, string name, unsigned int source, unsigned int PDFtype, unsigned int Ntype, map <string, unsigned int> ID);
void RearrangementInt(double *I, const double *q, const calc *cfg, const block *Block, vector<double*> FF, vector<double> SL, unsigned int Ntot);
#ifdef UseCUDA
...
void calcInt2D_OCL(cl_context OCLcontext, cl_device_id OCLdevice, cl_program OCLprogram, double ***I2D, double **I, const calc *cfg, const unsigned int *NatomType, cl_mem dra, const cl_mem *dFF, vector<double> SL, cl_mem dq);
...
#else
void calcInt2D(double ***I2D, double **I, const calc *cfg, const unsigned int *NatomType, const vector < vect3d <double> > *ra, vector <double*> FF, vector<double> SL, const double *q, unsigned int Ntot, int NumOMPthreads);
void calcIntDebye(double **I, const calc *cfg, const unsigned int *NatomType, const vector < vect3d <double> > *ra, vector <double*> FF, vector<double> SL, const double *q, unsigned int Ntot, int NumOMPthreads);
void calcIntPartialDebye(double **I, const calc *cfg, const unsigned int *NatomType, const vector < vect3d <double> > *ra, vector <double*> FF, vector<double> SL, const double *q, const block *Block, unsigned int Ntot, int NumOMPthreads);
void calcPDFandDebye(double **I, double **PDF, const calc *cfg, const unsigned int *NatomType, const vector < vect3d <double> > *ra, vector <double*> FF, vector<double> SL, const double *q, unsigned int Ntot, int NumOMPthreads);
#endif
int main(int argc, char *argv[]){
	calc cfg;
	int error = 0;
	unsigned int *NatomType=NULL, Ntot = 0;
	block *Block = NULL;
	...
	float4 *dra = NULL;//we use float4 structure for three coordinates to assure for global memory access coalescing
#elif UseOCL
	unsigned int DeviceNUM = 0, PlatformNUM=0;
	...
	Ntot = CalcAndPrintAtoms(&cfg, Block, &ra, &NatomType, ID);//creating the atomic ensemble
#ifdef UseCUDA
...
#endif
	switch (cfg.scenario) {
		case 0://calculating the 2D scatteing intensity
#ifdef UseCUDA
			calcInt2DCuda(DeviceNUM, &I2D, &I, &cfg, NatomType, dra, dFF, SL, dq);
#elif UseOCL
			calcInt2D_OCL(OCLcontext, OCLdevice, OCLprogram, &I2D, &I, &cfg, NatomType, dra, dFF, SL, dq);
#else
			calcInt2D(&I2D, &I, &cfg, NatomType, ra, FF, SL, q, Ntot,NumOMPthreads);
#endif
			if (!myid) {
			...
			break;
		case 1://calculating the 1D scatteing intensity using the Debye formula
#ifdef UseCUDA
			if (cfg.calcPartialIntensity) calcIntPartialDebyeCuda(DeviceNUM, &I, &cfg, NatomType, dra, dFF, SL, dq, Block);
			else calcIntDebyeCuda(DeviceNUM, &I, &cfg, NatomType, dra, dFF, SL, dq);
#elif UseOCL
			if (cfg.calcPartialIntensity) calcIntPartialDebyeOCL(OCLcontext, OCLdevice, OCLprogram, &I, &cfg, NatomType, dra, dFF, SL, dq, Block);
			else calcIntDebyeOCL(OCLcontext, OCLdevice, OCLprogram, &I, &cfg, NatomType, dra, dFF, SL, dq);
#else
			if (cfg.calcPartialIntensity) calcIntPartialDebye(&I, &cfg, NatomType, ra, FF, SL, q, Block, Ntot,NumOMPthreads);
			else calcIntDebye(&I, &cfg, NatomType, ra, FF, SL, q, Ntot,NumOMPthreads);
#endif
			if (!myid) {
			...
			break;
		case 2://calculating the 1D scatteing intensity using the pair-distribution histogram
#ifdef UseCUDA
			calcPDFandDebyeCuda(DeviceNUM, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
#elif UseOCL
			calcPDFandDebyeOCL(OCLcontext, OCLdevice, OCLprogram, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
#else
			calcPDFandDebye(&I, &PDF, &cfg, NatomType, ra, FF, SL, q, Ntot,NumOMPthreads);
#endif
			if (!myid) {
				if (cfg.rearrangement) RearrangementInt(I, q, &cfg, Block, FF, SL, Ntot);
				t2 = chrono::steady_clock::now();
			...
			break;
		case 3://calculating the pair-distribution function
#ifdef UseCUDA
			calcPDFandDebyeCuda(DeviceNUM, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
#elif UseOCL
			calcPDFandDebyeOCL(OCLcontext, OCLdevice, OCLprogram, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
			...
			if (!myid) {
				t2 = chrono::steady_clock::now();
				cout << "Total calculation time: " << chrono::duration_cast< chrono::duration < float > >(t2-t1).count() << " s" << endl;
				if (cfg.PrintPartialPDF) printPartialPDF(PDF + cfg.Nhist, cfg.Nhist, cfg.hist_bin, cfg.name, cfg.source, cfg.PDFtype, cfg.Ntype, ID);
				printPDF(PDF, cfg.Nhist, cfg.hist_bin, cfg.name,cfg.source,cfg.PDFtype);
			}
			break;
		case 4://calculating the 1D scatteing intensity using the pair-distribution histogram and pair-distribution function
#ifdef UseCUDA
			calcPDFandDebyeCuda(DeviceNUM, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
#elif UseOCL
			calcPDFandDebyeOCL(OCLcontext, OCLdevice, OCLprogram, &I, &PDF, &cfg, NatomType, dra, dFF, SL, dq);
...
#ifdef UseCUDA
	delDataFromDevice(dra, dFF, dq, cfg.Ntype);//deleting the data from the device memory
#elif UseOCL
	delDataFromDeviceOCL(OCLcontext, OCLprogram,dra,dFF,dq,cfg.Ntype);
#endif
	if (I2D!=NULL) {
	...
	if (cfg.source==1)	for (unsigned int i=0;i<FF.size();i++)	delete[] FF[i];
	delete[] Block;
	delete[] ra;
	delete[] NatomType;	
#ifdef UseMPI
